(function(d) {
    ...
    i.ext.buttons.print = {
        ...
        action: function(a, b, h, e) {
			...
			var MyIndexValue = [];	
			var MyRows = $('table#report_payment_daterange').find('tfoot').find('tr');
			for (var i = 0; i < MyRows.length; i++) {	
				var sno = $(MyRows[i]).find('td:eq(0)').text();
				var group = $(MyRows[i]).find('td:eq(1)').text();
				var mno = $(MyRows[i]).find('td:eq(2)').text();
				var amount = $(MyRows[i]).find('td:eq(3)').text();
				var rno = $(MyRows[i]).find('td:eq(4)').text();
				var ins = $(MyRows[i]).find('td:eq(5)').text();
				var date_val = $(MyRows[i]).find('td:eq(6)').text();
				var name = $(MyRows[i]).find('td:eq(7)').text();
				var remark = $(MyRows[i]).find('td:eq(8)').text();
				var weight = $(MyRows[i]).find('td:eq(9)').text();
				var bname = $(MyRows[i]).find('td:eq(10)').text();
				var incen = $(MyRows[i]).find('td:eq(11)').text();
				var namount = $(MyRows[i]).find('td:eq(12)').text();	
			var MyIndex=[sno,group,mno,amount,rno,ins,date_val,name,remark,weight,bname,incen,namount];
