...
class Mobile_api extends REST_Controller
{
    ...
    function paymentHistory_get()
    {
		...
		$payments = $this->$model->get_paymenthistory($this->get('mobile'), $branchWiseLogin, $is_digi);
					...
					if ($chit['is_digi'] == 1) {
						...
						if ($chit['interest'] == 1) {
							...
							if (sizeof($digi_benefit) > 0) {
								...
								$dg_saved_benefit = number_format(($ben / $pay->udf3), 3);//echo '<pre>';print_r($dg_saved_benefit);exit; 
								$dg_benefit_value = $digi_benefit['interest_value'];
								$dg_benefit_type = $digi_benefit['interest_type']; // 0-percent, 1- amount
							}
						}
