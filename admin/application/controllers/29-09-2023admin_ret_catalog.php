			...
			case "add":
...

				{
...

				    {
				        ...
				        $checkItemExist=$this->$model->checkReorderItemExist($items['id_branch'],$addData,$items['id_sub_design'],$items['weight_range'],$items['id_size']);
...

				        {
        					...
        					'id_wt_range'	        => $items['weight_range'],
...

		{
			...
			case 'list':
			 ...
			 case "Add":
					...
					$data=array('name'=>$shape,'description'=>$description , 'created_by'=>$this->session->userdata('uid'));

					$this->db->trans_begin();

					$this->$model->insert_shape($data);
	...
	public function selling_diamond_rate($type="",$id="",$status="",$quality_code_id="")
	{
        ...
        switch($type)
        {
            ...
            case 'delete':
                ...
                {
                    $this->db->trans_commit();
                    $this->session->set_flashdata('chit_alert', array('message' => 'Selling Diamond deleted successfully','class' => 'success','title'=>'Delete Diamond Rate'));
                    //echo 1;
                }
                else
                {
