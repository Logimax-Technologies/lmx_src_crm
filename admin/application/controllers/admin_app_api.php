...
class Admin_app_api extends REST_Controller
{
	...
	function convertDateFormat(?string $inputDate, string $currentFormat, string $desiredFormat): ?string {
		...
		//SSO Starts Here
	function test_sso_api_post(){
		// echo $_POST;
		// echo "<pre>";print("admin_app_api/test_sso_api");exit;
		$return_array = ['status'=> 'successed from logimax'];
		return json_encode($return_array);
	}
	public function login_sso_post(){

    $postData = $this->post("token");
	// echo "<pre>";print_r($postData);exit;
    if ($postData) {
        $this->response(array('status' => true,'token'=>$postData), 200);
        } else {
           	$this->response(array('status' => false,'token'=>$postData), 200);
        }
	}
	public function logimax_company_get(){
		$this->load->view('sso/dashboard', true);
	}

	public function sso_connection_post() {
		$payload = json_encode([
		...
		$sso_url = 'http://192.168.1.44:8000/login';
		...
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false); // Local testing
