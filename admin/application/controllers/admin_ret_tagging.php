...
//							'tax_group_id'			=> (isset($addData['tax_group_id']) ? $addData['tax_group_id'] :NULL ),
...

	{
			...
			//print_r($stone_details);exit;
...

    		  {
				...
				$lot_purchase_touch		= $reqdata['purchase_touch'];
		...
		else
		{
...

			{
...

				{
						...
						else if($tag_process==2 && $accInsert)
	...
	function get_printer_code($data) {
		...
		if($data['calculation_based_on'] == 3 || $data['calculation_based_on'] == 4) {
			...
			if($data['tax_type'] == 2) {
				...
				if(count($tax_details) > 0){
					...
					$base_value_tax = round($this->calculate_base_value_tax($sell_rate, $data['tgrp_id'], $tax_details),2);

					$base_value_amt = round(($sell_rate + $base_value_tax),2);

					$arrived_value_tax = round(($this->calculate_arrived_value_tax($base_value_amt, $data['tgrp_id'], $tax_details)),2);
		...
		if(trim($data['hu_id2']) != "" && trim($data['hu_id2']) != "-") {
		...
		$printer_code = 'CT~~CD,~CC^~CT~'. "\r\n" .
						...
						'^FT283,89^A0N,20,19^FH\^FD'.($data['purity_name']+0).'^FS'. "\r\n" .
