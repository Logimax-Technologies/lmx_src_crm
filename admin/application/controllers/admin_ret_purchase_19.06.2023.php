   ...
   function send_karigar_sms()
    ...
    function update_ratefix_approval()
	{
		...
		foreach ($insert_data as $value)
		{
            ...
            $this->db->trans_begin();
			$this->$model->updateData($Data,'rate_fix_id',$Data['rate_fix_id'],'ret_po_rate_fix');
		}
		if($this->db->trans_status()===TRUE)
        {
		 	$this->db->trans_commit();
  ...
  function karigarmetalissue($type="")
			        ...
			        if($insId)
			        {
			            ...
			            foreach($metal_details['metal'] as $key => $val)
			            {
			                ...
			                $metal_data=array(
			                                 ...
			                                 'issu_met_id_section'      =>$metal_details['id_section'][$key],
			                                 );
			               ...
			               if($status)
			               {
			                        ...
			                        $existData=array('id_section'=>$metal_details['id_section'][$key],'id_product'=>$metal_details['id_product'][$key],'design'=>$metal_details['id_design'][$key],'id_sub_design'=>$metal_details['id_sub_design'][$key],'id_branch'=>$ho['id_branch']);
    						        ...
    						        if($isExist['status'] == TRUE)
    						        {
                                        ...
                                        $section_nontag_log = array(
                                            ...
                                            "from_branch"	=> $ho['id_branch'],
                                            "to_branch"		=> NULL,
                                            "from_section"	=> $metal_details['id_section'][$key],
                                            "to_section"    => NULL,										
                                            "created_by"	=> $this->session->userdata('uid'),
                                            "created_on"	=> date('Y-m-d H:i:s'),
                                            "date"			=> $bill_date,
		...
		switch ($type) {
			...
			case 'save':
				...
				$data = array(
					...
					'amount'            => ($addData['type1_amt'] != '' ? $addData['type1_amt'] : ($addData['type2_amt'] != '' ? $addData['type2_amt'] : 0)),
					...
					'tax_amount'        => ($addData['tax_amount'] != '' ? $addData['tax_amount'] : 0),
					'tax_percentage'    => ($addData['tax_percentage'] != '' ? $addData['tax_percentage'] : NULL),
					'created_on'        => date("Y-m-d H:i:s"),
					'created_by'        => $this->session->userdata('uid'),
				);
				$this->db->trans_begin();
	...
	{   
		...
		switch($type)
		{
			...
			case 'ajax': 
				$list=$this->$model->get_weight_gain_loss_report($_POST); 
