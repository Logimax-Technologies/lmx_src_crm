...
class Admin_app_api extends REST_Controller
{
	...
	*/
	...
	public function authenticate_post()
	{
		...
		if ($device_id != "" && $username != '' && $userpwd != '')
		{
			$is_logged_in = $this->$model->check_device_logged_in($username, $device_id);

			if(!$is_logged_in) {

				if($this->$model->update_device_login_status($username, $device_id, 1)) {
				...
				{
					$result=array('result'=> FALSE ,'msg'=>'Error occured. Please try again.');
					$this->response($result, 200);
				}
			}
			else
			{
				$result=array('result'=> FALSE ,'msg'=>'Your device registered somewhere. Cannot login new device. Please contact administrator.');
				$this->response($result, 200);
			}
		}
		else
				...
				if($addData['is_non_tag']  && sizeof($addData['non_tag']) > 0){
					...
					foreach($addData['non_tag'] as $estNonTag) {
						...
						$arrayestNonTag =  array(
							...
							'id_sub_design'        => !empty($estNonTag->sub_design_id) && $estNonTag->sub_design_id != '' ? $estNonTag->sub_design_id : NULL, 
	...
	function printbill_get($billcode)
	{
		...
		$this->load->helper(array('dompdf', 'file'));
        $dompdf = new DOMPDF();
		$html = $this->load->view('billing/print/billing_soft_copy', $data,true);
	...
	function get_mc_va_limit_post() {
		...
		if($id_product > 0 && $id_design > 0 && $type > 0 && ($type == 1 ? $gross_wt > 0 && $id_sub_design > 0 && $lot_no > 0 : true)) {

			$mc_va_limit = $this->$model->get_mc_va_limit($id_product, $id_design, $id_sub_design, $gross_wt, $lot_no);
	...
	function getSections_get($branch) {
		...
		$result = $this->ret_catalog_model->get_sectionBranchwise($data);
