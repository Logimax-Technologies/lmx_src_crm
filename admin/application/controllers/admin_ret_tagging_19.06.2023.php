...
//							'tax_group_id'			=> (isset($addData['tax_group_id']) ? $addData['tax_group_id'] :NULL ),
			...
			case 'generate_barcode':
						...
						$zplCode = '^xa' . "\n" .'^fo100,100^a0n,36,36^fd' . $text1 . ' ' . $text2 . '^fs' . "\n" .'^fo100,200^b3n,N,100,Y,N^fd' . $barcodeData . '^fs' ."\n" .'^xz';

						// Set the file name
						$fileName = 'label.prn';

						// Set appropriate headers for download
						header('Content-Type: application/prn');
    		  ...
    		  if($allow_submit)
    		  {
				...
				foreach($reqdata['tags'] as $data)
				{
					...
					else if($bulk_edit_options == 7) 
					{
						...
						if(!$update_status)
							break;
					}else if($bulk_edit_options == 13)  //Updated calculation type
                    {
                        if(!empty($calc_type)){
                        	$upd_data=array(
    	    ...
    	    if($process_id)
    	    {
            	...
            	else if($report_type==3 && $tag_process==2) // PARTLY SALE
                {
                     foreach($reqdata as $tag)
                     {
                           ...
                           $this->$model->insertData(array('id_process'=>$process_id,'ref_no'=>$tag['tag_id'],'gross_wt' => $tag['gross_wt'],'net_wt'=> $tag['gross_wt'],'narration'=>$tag['narration'] ),'ret_acc_stock_process_details'); 
