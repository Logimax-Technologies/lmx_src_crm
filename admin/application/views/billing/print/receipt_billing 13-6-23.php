		...
		<span class="PDFReceipt">
			...
			<div class="maindiv" style="margin-top:0px !important; ">
				...
				<div class="right">
					<div style="text-align: left !important;width: 100%; display: inline-block; font-weight:bold">
						...
						<label><?php $disp_label = $billing['bill_type'] == 15 ? 'Ack No' : 'INVOICE NO';
								echo '<div class="addr_brch_labels" style="height: 18px;">' . $disp_label . '</div><div class="addr_brch_values" style="height: 18px;">:&nbsp;&nbsp;' . $billing['invoice_no'] . "</div><br>"; ?>
						</label>
					...
					<div align="center" style="display: inline-block; width: 22%;padding-left:40px;font-weight: bold; text-align:center !important; padding-top:70px">
						...
						-->
							<?php echo $billing['bill_type'] != 15 ? "  " .
								($billing['bill_type'] == 1 ? ($billing['is_to_be'] == 1 ? 'To Be Sales Bill' : ($billing['is_credit'] == 1 ? 'Credit Sales Bill' : 'Sales Bill')) : ($billing['bill_type'] == 2   ? ($metal_name != '' ? 'Sales & Old ' . $metal_name . ' Purchase ' : 'Sales & Purchase') : ($billing['bill_type'] == 14 ? "Sales Return Transfer" : ($billing['bill_type'] == 3 ?   'Sales & Credit Sales ' : ($billing['bill_type'] == 4 ? ($metal_name != '' ? 'Old ' . $metal_name . ' Purchase Bill' : 'Purchase Bill') : ($billing['bill_type'] == 5 ? 'Order Advance' : ($billing['bill_type'] == 6 ? 'Advance' : ($billing['bill_type'] == 7 ? 'CREDIT NOTE' : ($billing['bill_type'] == 9 ? 'Sales Bill' : ($billing['bill_type'] == 10 ? 'Chit PreClose' : ($billing['bill_type'] == 11 ? 'Repair Order' : ($billing['bill_type'] == 13 ? 'Sales Transfer' : ($billing['bill_type'] == 15 ? 'Approval Bill Acknowledgement' : 'Credit Collection'))))))))))))) . "" : ($billing['bill_status'] == 2 ? '- Cancelled' : '') ?>
						</label>
						<!-- DONE -->
					</div>
				</div>
				<div class="content-wrapper" style="margin-top:20px;">
					<div class="box">
						<div class="box-body">
												...
												<table id="pp" class="table text-center">
													...
													<?php if ($total_sgst > 0) { ?>
														<tr>
															...
															<td class="alignLeft"><?php echo (sizeof($est_other_item['tax_details'])==1 ? ($est_other_item['item_details'][0]['tax_percentage'] / 2) . '%'  :'')?></td>
													...
													<?php if ($total_igst > 0) { ?>
														<tr>
															...
															<td class="alignLeft"><?php echo (sizeof($est_other_item['tax_details'])==1 ? ($est_other_item['item_details'][0]['tax_percentage']) . '%'  :'')?></td>
											...
											<div class="table-responsive">
												<table id="pp" class="table text-center">
													<thead style="text-transform:uppercase;font-size:19px;">
														...
														<tr>
														    <td><hr class="header_dashed" style="width:1500%;"></td>
														</tr>
														<tr>
							...
							<?php if (sizeof($est_other_item['return_details']) > 0) { ?>
								<span style="font-weight: bold">CREDIT NOTE INVOICE NO : <?php echo  $billing['branch_code'] . '-SR-' . $billing['metal_code'] . '-' . $billing['s_ret_refno']; ?></span>
								<table id="pp" class="table text-center">
									<!--	<thead> -->
									<tr>
										<td>
									...
									?>
										<tr>
											<td colspan="9" style="text-transform:uppercase; font-weight: bold">Refer sale bill no : <?php echo $items['sales_ref_no'] . ' Dt ' . $items['ref_bill_date'] ?> </td>
										</tr>
										<tr>
					...
					<?php } ?>
					<?php
					if($cash_amt > 0 || $chq_amt>0 || $card_amt > 0 || $rtgs_amt > 0 || $imps_amt > 0 || $upi_amt > 0 || $adv_adj > 0 || $gift_amount > 0 || $billing['advance_deposit']> 0 || $due_amount != 0) { ?>
						<!--<table>
							<tr>
								<?php if ($due_amount != 0) { ?>
									<td>DUE AMOUNT</td>
						...
						<table id="pp" class="pay_mode_totals table text-left" style="width:100%;">
							...
							<tbody>
								...
								<?php } ?>
								<?php if (($ord_adj_amt != 0 || $billing['adv_adj_amt'] != 0) && $deposit_bill_id == '' && $billing['ord_adv_adj']=='') { ?>
									<tr>
								...
								foreach ($receiptDetails as $val) {?>
									<tr>
										...
										<td style="width:66%"><?= $val['bill_no']!='' ? (' Refer Receipt No : '.$val['bill_no'] ) : '0'?></td>
									</tr>
								<?php } }?>
								...
								<?php if(sizeof($billing['ord_adv_adj']) > 0){
									foreach($billing['ord_adv_adj'] as $ord_adj){ ?>
										<tr>
												<td style="width:24%"><?php echo $ord_adj['deposit_type'] == 1 ? ($ord_adj['metal'] != '' ? 'BY CUST OWN ' . $ord_adj['metal'] . ' EX' : 'BY CUST OWN EX') : 'ADV ADJ'  ?></td>
												<td style="width:10%"class="alignRight"><?php echo moneyFormatIndia(number_format($ord_adj['utilized_amt'], 2, '.', '')); ?></td>
												<td style="width:66%"><?php echo  $ord_adj['deposit_type'] == 1 ? ($ord_adj['adjusted_bill_no'] != '' ? '&nbsp;&nbsp; Ref Pur No : ' . $ord_adj['adjusted_bill_no'] : '-') :  '' ?>;</td>
											</tr>
									<?php }
								}?>
								<?php if ($chit_adj != 0  && ($billing['bill_type'] != 10)) { ?>
								...
								<?php if (sizeof($billing['sales_rtn_bill_no']) > 0) {
									foreach ($billing['sales_rtn_bill_no'] as $adj) { ?>
										<tr>
										<td style="width:24%"><?php echo $adj['sales_ref_no'] != '' ? 'REFER SALES BILL : ' : ($adj['order_adv_ref_no']!='' ? 'REFER ORDER ADVANCE NO : ':'' ) ?></td>
										<td style="width:10%" class="alignRight"><?php echo $adj['sales_ref_no']!='' ? $adj['sales_ref_no']: ($adj['order_adv_ref_no']!='' ? $adj['order_adv_ref_no'] : ''); ?></td>
											<td style="width:66%"></td>
										</tr>
								<?php	}
								} ?>
