 ...
 /*product functions here */
  ...
  function get_weight($id)
...

   {
	   ...
	   AND id_wt_range=".$set['weight_range']." 

	   AND max_pcs=".$set['max_pcs']." 

	   AND min_pcs=".$set['min_pcs']." 

	   ".($set['size']!='' ? " and size=".$set['size']."" :'')."
 ...
 function get_ref_no($id)
 {
   ...
   /*Delivery functions here */
...

	{
		...
		$sql=$this->db->query("SELECT mc.id_old_metal_cat, mc.id_metal, mc.id_old_metal_type, mc.old_metal_cat, mc.old_metal_perc, t.id_metal_type,t.metal_type,mc.old_metal_discount, m.metal
	...
	function check_weight_range($from_weight,$to_weight,$value,$id_product,$desc)
	{
		...
		$sql=$this->db->query("SELECT  w.id_weight,w.from_weight,  w.to_weight ,w.value,w.id_product, w.weight_description from  ret_weight w
		 ...
		 ".($from_weight!='' && $from_weight>0 ? "and  w.from_weight <= ".$from_weight." and w.to_weight >= ".$from_weight."":"")."
		
		 ".($to_weight!='' && $to_weight>0 ? "and  w.from_weight <= ".$to_weight." and w.to_weight >= ".$to_weight."":"")."
...

	{
	    ...
	    $sql=$this->db->query("SELECT p.pro_id,p.product_name, p.sales_mode, p.purchase_mode,p.stock_type,p.tax_type,p.tax_group_id,p.job_work_receipt_tgrp_id,p.purchase_tax_type,p.stone_type
...

	{
		...
		WHERE  TABLE_NAME = 'metal_rates'  AND COLUMN_NAME in ('goldrate_22ct','goldrate_24ct','goldrate_24ct','silverrate_1gm','silverrate_1kg','platinum_1g','goldrate_18ct','mjdmasilverrate_1gm','market_gold_18ct','market_gold_20ct','market_gold_995','mjdmasilverrate_999');");
...
//Master:Diamond Quality rate Ends
...

	{
	    ...
	    $sql=$this->db->query("SELECT s.id_selling_settings,p.product_name,des.design_name,IFNULL(subDes.sub_design_name,'') as sub_design_name,if(s.mc_type=1,'  / Piece',if(s.mc_type=2,'/ Gram',' % Price')) as mc_type,if(s.type=1,'Fixed','Weight Range') as type,IFNULL(s.mc_value,'-') as mc,IFNULL(s.mc_min,'-') as min_mc,IFNULL(s.wastage_perc ,'-') as va,IFNULL(s.wastag_min,'-') as min_va,b.name,s.type as wastage_type,s.margin_mrp
