...

	{
	   ...
	   $sql=$sql="Select IFNULL(sum(payment_amount),0) as paid,p.id_branch,b.name as branch_name,b.short_name,
	   			...
	   			left join branch b on b.id_branch=p.id_branch 
	   			 ...
	   			 where b.active = 1 and (date(p.date_payment) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') and p.payment_status=1
...

	{
	  ...
	  $result=0;	
	   ...
	   From payment p
	   		...
	   		LEFT join branch b on (b.id_branch=p.id_branch and b.active = 1)
		...
		switch($filterBy){
			...
			case 'MOB': //Joined thro Mobile App
			         ...
			         $sql=$sql." where  b.active = 1 and p.added_by = 2 and payment_status=1  and (date(p.date_payment) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."')  
			...
			case 'WEB': //Joined thro Web App
			         ...
			         $sql=$sql." where b.active = 1 and p.added_by = 1 and payment_status=1 and (date(p.date_payment) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') 
			...
			case 'ADMIN': //Joined thro Admin
			          ...
			          $sql=$sql." where b.active = 1 and p.added_by = 0 and payment_status=1 and (date(p.date_payment) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') 
			...
			case 'COLLECTION': //Joined thro Admin
			          ...
			          $sql=$sql." where b.active = 1 and p.added_by = 3 and payment_status=1 and (date(p.date_payment) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') 
			 ...
			 $sql="SELECT count(sa.id_scheme_account)as count,b.name
				...
				LEFT join branch b on (sa.id_branch=b.id_branch)
        			...
        			WHERE NOT EXISTS(
						...
						) and  b.active = 1 AND sa.paid_installments=0 AND sa.is_closed=0 AND (date(sa.date_add) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') 
...
//function to get customer list between daterange 
 ...
 function customer_detail_bydate($from_date="",$to_date="",$id_branch="")
		 ...
		 $sql="SELECT sa.id_scheme_account
				...
				LEFT join branch b on (sa.id_branch=b.id_branch)
        			...
        			WHERE  NOT EXISTS(
						...
						) and b.active = 1 AND sa.paid_installments=0 AND sa.is_closed=0 	".($dashboard_branch!=0 ? "and sa.id_branch=".$dashboard_branch :'')." ".($uid!=1 ? ($branchWiseLogin==1? ($id_branch!='' ?  " and( sa.id_branch=".$id_branch." or b.show_to_all=1 )":''):''):'')." 
...

	{
		...
		IFNULL((select count(pay.id_payment) from payment pay where pay.id_employee=p.id_employee and pay.added_by=3 and  (date(pay.date_payment) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') 

		".($id_branch!='' && $id_branch!=0 && $id_branch != null ? " and pay.id_branch=".$id_branch:'')."
		...
		(date(p.date_payment) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') 

		".($id_branch!='' && $id_branch!=0 && $id_branch != null ? " and p.id_branch=".$id_branch:'')." 

		and p.payment_status=1 and p.added_by=3  GROUP by p.id_employee";
	...
	function schWise_accounts($from_date,$to_date,$id_branch){
    ...
    function get_oldclosed_amt($id_branch){
	    ...
	    $closed_accounts=$this->db->query("SELECT SUM(sa.closing_amount) as all_closing
                                         FROM scheme_account sa
	...
	function get_newclosed_amt($id_branch){
	    ...
	    $closed_accounts=$this->db->query("SELECT SUM(sa.closing_amount) as range_closing
                                        FROM scheme_account sa
                                        WHERE  sa.is_closed = 1 and date(sa.closing_date) BETWEEN '".$from_date."' AND '".$to_date."'
                                        ".($id_branch>0 ? " and sa.id_branch=".$id_branch."":'')."
                                        "); 
