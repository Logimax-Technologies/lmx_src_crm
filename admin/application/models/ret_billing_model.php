...
/*
...

	{
		...
		ifnull(tag.quality_id,'') as quality_id,pro.stone_type,ifnull(tag.uom_gross_wt,'') as uom_id,ifnull(tag.stone_calculation_based_on,'') as stone_calculation_based_on,est_other_metal.tag_other_itm_amount, addr.id_state, addr.id_country
	...
	*/
	...
	function get_active_bill_list($bill_id, $branch, $fin_year)
	{
		...
		$sql = $this->db->query("SELECT p.id_pay_device,p.payment_mode,IFNULL(p.payment_amount,0) as payment_amount,p.bill_id,
			...
			b.bill_id as bill, b.id_employee as id_employee, b.bill_cus_id as bill_cus_id, b.pan_no AS pan_no, b.gst_number AS gst_no, b.aadhar_no AS aadhaar_no, b.billing_for AS billing_for
			from ret_billing b
			left join ret_billing_payment p on p.bill_id=b.bill_id
			left join customer c on c.id_customer=b.bill_cus_id
			LEFT JOIN branch br on br.id_branch=b.id_branch
