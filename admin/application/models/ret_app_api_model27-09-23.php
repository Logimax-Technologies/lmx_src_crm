...
function getAvailableCustomers($SearchTxt){
	...
	function getUserMenus($id_profile){ 
        ...
        $menus = array ( 
        			...
        			array("id" => 9, "parent_id" => 0, "title" => "Cart", "icon" => "cart", "component" => "CartPage", "show"  => 0, "show_in_dash"  => "1"), //Disabled for ams,
...

	{
		...
		$sql_created = $this->db->query("SELECT count(estimation_id) AS created, IFNULL(sum(estitm.amt),0) AS created_amt, IFNULL(sum(estitm.nwt),0) AS created_wgt 
										...
										WHERE estitm.esti_id IS NOT NULL AND ( date(est.estimation_datetime) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') ".($id_emp > 0 && 'e.id_profile' > 3 ? ' AND created_by = '.$id_emp : '').' '.($id_branch > 0 ? ' AND id_branch = '.$id_branch : ''));
		...
		$sql_sold = $this->db->query("SELECT 
										...
										WHERE bill.bill_status = 1 AND (DATE(bill.bill_date) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') ".($id_emp > 0 && 'e.id_profile' > 3 ? ' AND created_by = '.$id_emp : '').' '.($id_branch > 0 ? ' AND est.id_branch = '.$id_branch : ''));
...

	{
		...
		$sql_created = $this->db->query("SELECT 
										...
										WHERE esti_om.est_id IS NOT NULL AND ( date(est.estimation_datetime) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."') ".($id_emp > 0 && 'e.id_profile' > 3 ? ' AND created_by = '.$id_emp : '').' '.($id_branch > 0 ? ' AND id_branch = '.$id_branch : ''));
    ...
    function getCurrentDayEstimationsByBranch($branchId="", $empId){
        ...
        if($branchId != "" && $empId != ""){
            $estquery = $this->db->query("SELECT estimation_id, esti_no, esti_for, date_format(estimation_datetime, '%d-%m-%Y %H:%i:%s') as estimatedon, 
                                    ...
                                    total_cost, est.id_branch, cus.firstname, cus.mobile, IF(IFNULL(est.estbillid,0) > 0, 1, 0) AS is_billed   
                                    FROM ret_estimation as est 
            ...
            $emp_filter     =  $empId !='' ? "est.created_by = '".$empId."' AND " : '';
             $estquery = $this->db->query("SELECT estimation_id, esti_no, esti_for, date_format(estimation_datetime, '%d-%m-%Y %H:%i:%s') as estimatedon, 
                                    ...
                                    (SELECT sum(gross_wt) as wt FROM ret_estimation_items WHERE esti_id = estimation_id) as totwt, IF(IFNULL(est.estbillid,0) > 0, 1, 0) AS is_billed 
                                    FROM ret_estimation as est 
                                    LEFT JOIN customer as cus ON cus.id_customer = est.cus_id 
	...
	function get_totalprice($tag, $rates, $rate_fields, $tax_details) {
		...
		if($calculation_based_on == 0) { 
			...
			$rate_with_mc = ($rate * ($wast_wgt + $net_wt)) + $mc_type + $stone_price + $certification_cost + $charge_value;
