	 ...
	 $id_branch=$this->input->post('id_branch');
	   ...
	   /*$sql="Select count(id_scheme_account) as total,b.id_branch,b.name as  branch_name from ".self::ACC_TABLE." s 
	        ...
	        from ".self::ACC_TABLE." s 
	   		...
	   		where  b.id_branch is not null AND s.active=1 and s.is_closed=0 and s.scheme_acc_number IS NOT NULL   and  p.payment_status = 1 AND (date(s.start_date) BETWEEN '".date('Y-m-d',strtotime($from_date))."' AND '".date('Y-m-d',strtotime($to_date))."')
