...

{
...

	{
		...
		$items_query = $this->db->query("SELECT est_item_id, esti_id, item_type, 
                       ...
                       sub.sub_design_name,mt.metal as metal_name,IFNULL(est.est_rate_per_grm,0) as est_rate_per_grm,c.id_metal as metal_type,c.is_916,c.name as cat_name,IFNULL(est.id_section,'') as id_section
...

    {
        ...
        IFNULL(tag.tag_purchase_cost,0) as tag_purchase_cost,IFNULL(sec.section_name,'') as section_name,IFNULL(tag.hu_id,'') as huid,tag.id_section
	...
	*/
...

    {
        ...
        IFNULL(tagOtherMetal.tag_other_itm_amount,0) as tag_other_itm_amount,cus.rate_per_gram as order_rate, CONCAT(s.value, ' ', s.name) as size_name,c.rate_type,IFNULL(sec.section_name,'') as section_name,IFNULL(tag.hu_id,'') as huid,tag.id_section
...

	{
        ...
        IFNULL(fixed_rate.item_cost,0) as tot_fixed_rate,IFNULL(active_acc.tot_acc,0) as active_acc,IFNULL(DATE_FORMAT(max(bill.bill_date),'%d-%m-%Y'),'-') as last_billdate,z.name as zone_name,addr.id_country,addr.id_state
...

   {
        ...
        ".($data['id_section']!='' && $data['id_section'] > 0 ? "AND rn.id_section=".$data['id_section']."":"")."
   ...
   function advance_details_order_details($orderno,$id_branch)
    ...
    { 
		...
		$items_query = $this->db->query("SELECT est_item_id, esti_id, item_type, item_emp_id,c.scheme_closure_benefit,
                       ...
                       IFNULL(concat(sz.value,'',sz.name),'') as size,sz.id_size,pro.tgrp_id as tax_group_id,IFNULL(tag.tag_purchase_cost,0) as tag_purchase_cost,r.rate_field,r.market_rate_field,IFNULL(sec.section_name,'') as section_name,IFNULL(tag.hu_id,'') as huid,tag.id_section
...

    {
        ...
        $items_query = $this->db->query("SELECT est_item_id, esti_id, item_type,est.id_sub_design,c.scheme_closure_benefit,
                       ...
                       sub.sub_design_name,mt.metal as metal_name,IFNULL(est.est_rate_per_grm,0) as est_rate_per_grm,c.id_metal as metal_type,c.is_916,c.name as cat_name,est.id_section as id_section
