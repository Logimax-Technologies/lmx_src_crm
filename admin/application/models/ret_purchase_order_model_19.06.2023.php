...
class Ret_purchase_order_model extends CI_Model
{
	...
	function get_purchase_qc_details($data)
	{
        ...
        IFNULL(qc_det.passed_nwt,0) as recd_nwt,p.ref_no,qc_det.status,qc_det.is_lot_created
        FROM ret_po_qc_issue_process p 
	...
	function getKarigarMetalIssueList()
	{
	    ...
	    IFNULL(c.pur_no,'-') as pur_no,if(i.bill_status=1,'Success','Cancelled') as bill_status,IFNULL(i.remark,'') as remark
        FROM ret_karigar_metal_issue i 
	...
	function get_available_stock_details($data)
	{
        ...
        c.name as category_name,c.id_ret_category as cat_id,c.tgrp_id,nt.id_section
   ...
   function get_po_paymentDetails($pay_id)
    ...
    function get_qc_receipt_lot()
    {
        ...
        (IFNULL(SUM(d.passed_pcs),0)-IFNULL(lot.no_of_piece,0)) as blc_pcs
        FROM ret_po_qc_issue_details d 
        LEFT JOIN ret_po_qc_issue_process q ON q.qc_process_id = d.qc_process_id
        LEFT JOIN ret_purchase_order_items r ON r.po_item_id = d.po_item_id
        LEFT JOIN ret_purchase_order p ON p.po_id = r.po_item_po_id
        LEFT JOIN (SELECT IFNULL(SUM(ld.no_of_piece),0) as no_of_piece,l.po_id
        ...
        WHERE d.status = 1 AND r.is_halmarked = 1
    ...
    {
        $sql = $this->db->query("SELECT p.po_id,p.po_ref_no,(IFNULL(SUM(r.item_pure_wt),0)-IFNULL(rc.fixed_weight,0)-IFNULL(ratefix.fixed_wt,0)-IFNULL(po_ret.pur_ret_pur_wt,0)) as item_pure_wt
        FROM ret_purchase_order p 
        LEFT JOIN ret_purchase_order_items r ON r.po_item_po_id = p.po_id
        LEFT JOIN(SELECT IFNULL(SUM(c.weight),0) as fixed_weight,c.po_id
        FROM ret_supplier_rate_cut c 
        WHERE c.status = 1 AND c.rate_cut_type = 2
    ...
    function get_headoffice_valut_report($data)
    {
        ...
        $sql = $this->db->query("SELECT p.po_ref_no,date_format(p.po_date,'%d-%m-%Y') as podate,k.firstname as karigar_name,
        round((IFNULL(blc_inw.inw_pcs,0)-IFNULL(blc_ret.pur_ret_pcs,0)-IFNULL(blc_tag.tag_pcs,0)),0) as blc_pcs,
        round((IFNULL(blc_inw.inw_gwt,0)-IFNULL(blc_ret.pur_ret_gwt,0)-IFNULL(blc_tag.tag_gwt,0)),3) as blc_gwt,
        round((IFNULL(blc_inw.inw_nwt,0)-IFNULL(blc_ret.pur_ret_nwt,0)-IFNULL(blc_tag.tag_nwt,0)),3) as blc_nwt,
        round((IFNULL(blc_stn.po_stone_wt,0)-IFNULL(blc_ret_stn.ret_stone_wt,0)-IFNULL(blc_tag_stn.stn_wt,0)),3) as blc_diawt,
        IFNULL(inw.inw_pcs,0) as inw_pcs,IFNULL(inw.inw_gwt,0) as inw_gwt,IFNULL(inw.inw_nwt,0) as inw_nwt,IFNULL(inw_stn.po_stone_wt,0) as inw_diawt,
        IFNULL(ret.pur_ret_pcs,0) as pur_ret_pcs,IFNULL(ret.pur_ret_gwt,0) as pur_ret_gwt,IFNULL(ret.pur_ret_nwt,0) as pur_ret_nwt,IFNULL(ret_stn.ret_stone_wt,0) as ret_diawt,
        IFNULL(lot.lot_pcs,0) as lot_pcs,IFNULL(lot.lot_gwt,0) as lot_gwt,IFNULL(lot.lot_nwt,0) as lot_nwt,IFNULL(lot_stn.stone_wt,0) as lot_dia_wt,
        IFNULL(tag.tag_pcs,0) as tag_pcs,IFNULL(tag.tag_gwt,0) as tag_gwt,IFNULL(tag.tag_nwt,0) as tag_nwt,IFNULL(tag.tag_nwt,0) as tag_nwt,IFNULL(tag_stn.stn_wt,0) as tag_dia_wt
        FROM ret_purchase_order p 
        LEFT JOIN ret_karigar k ON k.id_karigar = p.po_karigar_id
        LEFT JOIN ret_grn_entry g ON g.grn_id = p.po_grn_id
        LEFT JOIN (SELECT IFNULL(SUM(itm.gross_wt),0) as inw_gwt,IFNULL(SUM(itm.net_wt),0) as inw_nwt,IFNULL(SUM(itm.no_of_pcs),0) as inw_pcs,p.po_id
            FROM ret_purchase_order_items itm
            LEFT JOIN ret_purchase_order p ON p.po_id = itm.po_item_po_id
            WHERE p.bill_status = 1 and p.is_approved = 1
        ...
        LEFT JOIN(SELECT IFNULL(SUM(d.no_of_piece),0) as lot_pcs,IFNULL(SUM(d.gross_wt),0) as lot_gwt,IFNULL(SUM(d.net_wt),0) as lot_nwt,l.po_id
            FROM ret_lot_inwards l 
            LEFT JOIN ret_lot_inwards_detail d ON d.lot_no = l.lot_no
        ...
        GROUP BY t.product_id,p.po_id) as blc_tag_stn ON blc_tag_stn.po_id = p.po_id
        ...
        $response_data = $sql->result_array();
    	foreach($response_data as $key => $val){
    	    $closing_pcs = number_format(($val['blc_pcs']+$val['inw_pcs']-$val['pur_ret_pcs']-$val['tag_pcs']),0,'.','');
    	    $closing_gwt = number_format(($val['blc_gwt']+$val['inw_gwt']-$val['pur_ret_gwt']-$val['tag_gwt']),3,'.','');
    	    $closing_nwt = number_format(($val['blc_nwt']+$val['inw_nwt']-$val['pur_ret_nwt']-$val['tag_nwt']),3,'.','');
    	    $closing_diawt = number_format(($val['blc_diawt']+$val['inw_diawt']-$val['ret_diawt']-$val['tag_dia_wt']),3,'.','');
    	    if($closing_pcs!=0)
    	    {
    	        $return_data[] =   array(
    	                            ...
    	                            'blc_pcs'       =>$this->moneyFormatIndia($val['blc_pcs']),
    	                            'blc_gwt'       =>$this->moneyFormatIndia($val['blc_gwt']),
    	                            'blc_nwt'       =>$this->moneyFormatIndia($val['blc_nwt']),
    	                            'blc_diawt'     =>$this->moneyFormatIndia($val['blc_diawt']),
    	                            'inw_pcs'       =>$this->moneyFormatIndia($val['inw_pcs']),
    	                            'inw_gwt'       =>$this->moneyFormatIndia($val['inw_gwt']),
    	                            'inw_nwt'       =>$this->moneyFormatIndia($val['inw_nwt']),
    	                            'inw_diawt'     =>$this->moneyFormatIndia($val['inw_diawt']),
    	                            'pur_ret_pcs'   =>$this->moneyFormatIndia($val['pur_ret_pcs']),
    	                            'pur_ret_gwt'   =>$this->moneyFormatIndia($val['pur_ret_gwt']),
    	                            'pur_ret_nwt'   =>$this->moneyFormatIndia($val['pur_ret_nwt']),
    	                            'ret_diawt'     =>$this->moneyFormatIndia($val['ret_diawt']),
    	                            'lot_pcs'       =>$this->moneyFormatIndia($val['lot_pcs']),
    	                            'lot_gwt'       =>$this->moneyFormatIndia($val['lot_gwt']),
    	                            'lot_nwt'       =>$this->moneyFormatIndia($val['lot_nwt']),
    	                            'lot_dia_wt'    =>$this->moneyFormatIndia($val['lot_dia_wt']),
    	                            'tag_pcs'       =>$this->moneyFormatIndia($val['tag_pcs']),
    	                            'tag_gwt'       =>$this->moneyFormatIndia($val['tag_gwt']),
    	                            'tag_nwt'       =>$this->moneyFormatIndia($val['tag_nwt']),
    	                            'tag_dia_wt'    =>$this->moneyFormatIndia($val['tag_dia_wt']),
    	                            'closing_pcs'   =>$this->moneyFormatIndia($closing_pcs),
    	                            'closing_gwt'   =>$this->moneyFormatIndia($closing_gwt),
    	                            'closing_nwt'   =>$this->moneyFormatIndia($closing_nwt),
    	                            'closing_diawt' =>$this->moneyFormatIndia($closing_diawt),
    	                            ); 
    	    }
