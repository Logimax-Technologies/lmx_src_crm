...
/* -- / coded by ARVK -- */       
...

    {
    	...
    	$userType = $this->session->userdata('profile');
		 ...
		 $this->db->select('employee.id_collection_device,ed.device_uuid,employee.enable_chit_collection,employee.enable_estimation_app,employee.mobile,employee.emp_code,employee.id_employee,employee.firstname,employee.lastname,department.name as dept,employee.username,profile.profile_name as usertype,employee.active,branch.name as branch_name');

         //$this->db->from('employee');
         $this->db->from('employee left join employee_devices ed on employee.id_collection_device=ed.id_collection_device');
...
*/
   ...
   function getActiveemployee()
   {
	...
	{
		$sql="SELECT e.id_employee,e.firstname,IFNULL(e.lastname,'') as lastname,IFNULL(e.emp_code,'-') as emp_code,IFNULL(ed.device_uuid,'') as device_uuid from employee e 
		left join employee_devices ed on e.id_collection_device=ed.id_collection_device
		 where e.active=1";
	...
	public function get_employee_name_byid($emp_id="")
	{
		...
		$sql="SELECT e.id_employee,e.firstname,IFNULL(e.lastname,'') as lastname,IFNULL(e.emp_code,'-') as emp_code,IFNULL(ed.device_uuid,'') as device_uuid,e.enable_chit_collection,e.enable_estimation_app,e.id_collection_device from employee e left join employee_devices ed on e.id_collection_device=ed.id_collection_device where active=1 and e.id_employee=".$id;
		
		return $this->db->query($sql)->row_array();
	}
	public function update_device_status($id,$status,$uuid="", $for_estimation_app_count = 0)
	{
		$field_name = "";

		if($for_estimation_app_count == 1) {

			$field_name = "enable_estimation_app";

		} else {

			$field_name = "enable_chit_collection";

		}

		$sql="UPDATE employee set ".$field_name."=".$status."";
		
		if($uuid!='' && $uuid!=null) {

			$sql.=",id_collection_device='".$uuid. "' where id_employee=".$id." and active=1";
		
		} else {

			$sql.=" where id_employee=".$id." and active=1";
		}
	...
	function get_enabled_device_count($for_estimation_app_count = 0)
	{
		...
		$sql = "SELECT count(id_employee) as count FROM `employee` WHERE ".$field_name." > 0 and active=1";
		
		return $this->db->query($sql)->row()->count;
	}
	public function get_employee_name_bydevice($uuid, $for_estimation_app_count = 0)
	{
		$field_name = "";
		...
		$sql="SELECT e.id_employee,e.firstname,IFNULL(e.lastname,'') as lastname,IFNULL(e.emp_code,'-') as emp_code,IFNULL(ed.device_uuid,'') as device_uuid,e.enable_chit_collection,e.id_collection_device from employee e left join employee_devices ed on e.id_collection_device=ed.id_collection_device where active=1 and e.".$field_name."=1 and e.id_collection_device=".$uuid;
