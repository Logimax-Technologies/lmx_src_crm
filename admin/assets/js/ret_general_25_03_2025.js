...
$('#add_newcutomer').click(function (event) {
	...
	if(ctrl_page[1]=='billing' || ctrl_page[1]=='estimation' || ctrl_page[1]=='receipt'){
...
$(document).on('change','#pan,#ed_pan_no,#kyc_pan',function(){
...

	{
...

		{
			...
			$("#pan,#ed_pan_no,#kyc_pan").focus();

			$("#pan,#ed_pan_no,#kyc_pan").val("");

		}else

		{

			checkPANAvail(this.value);
...
function checkPANAvail(pan_number)
...
$(document).on('change','#gst_no,#gst_number,#gst_number_karigar',function(){
...

	{
		...
		if (!gstinformat.test(gst)) {
			...
			$('#gst_no,#gst_number,#gst_number_karigar').val("");

			$.toaster({ priority: 'danger', title: 'Warning!', message: '' + "</br>" + 'Enter Valid GST NO' });

			$('#gst_no,#gst_number,#gst_number_karigar').focus();
...
function checkGSTAvail(gst_number)
...
$(document).on('change','#aadharid,#kyc_aadhar',function(){
...

	{
...

		{
			...
			$("#aadharid,#kyc_aadhar").focus();

			$("#aadharid,#kyc_aadhar").val("");

		}else

		{

			checkAADHARAvail(this.value);
...
function checkAADHARAvail(aadhar_number)
