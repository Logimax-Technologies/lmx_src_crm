...
$(document).ready(function() {
...

	{
		...
		if(enable_device_limit>0 && login_profile==1)
		{
			  	  ...
			  	  {  	
					  	...
					  	oTable = $('#emp_list').dataTable({
				                ...
				                "aoColumns": [{ "mDataProp": "id_employee" },
					                ...
					                //Lines Added by Durga starts here 17-07-2023
									{ "mDataProp": function ( row, type, val, meta )
									{
										if(row.device_uuid!='' && row.device_uuid!=null)
										{
									...
									{ "mDataProp": function ( row, type, val, meta )
									{
										if(row.enable_chit_collection==1)
										{
									...
									//Lines Added by Durga ends here 17-07-2023

									{ "mDataProp": function ( row, type, val, meta )
									{
										if(row.enable_estimation_app==1)
										{
											return "<i class='fa fa-check' style='color:green'></i>";
										}
										else
...
function get_employee_name_byid(id)
{
	$.ajax({
			 url:base_url+ "index.php/admin_employee/get_employee_name_byid",
			 type:"POST",
			 dataType:'json',
			 data:{'id_employee':id},
			 async:false,
			 	  success:function(data)
				  {
						$("#device_uuid").val(data.id_collection_device);
						$("#device_select").select2("val",data.id_collection_device);
						if(data.enable_chit_collection==1)
						{
							$('#enable_device').prop('checked', true);
						}
						else
						{
							$('#enable_device').prop('checked', false);
						}
						if(data.enable_estimation_app==1)
						{
							$('#enable_estimation_device').prop('checked', true);
						}
						else
						{
							$('#enable_estimation_device').prop('checked', false);
						}
				  }
			});
}
