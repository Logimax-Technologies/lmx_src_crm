        ...
        $.each(karigar_stone_details,function(key,items)
        {
            if(items.id_karigar==$('#select_karigar').val() && curRow.find('.stones_type').val()==items.stone_type && curRow.find('.stone_id').val()==items.stone_id)
            {
                if(curRow.find('.stones_type').val() == 1 && (curRow.find('.quality_id').val()==items.quality_id))
                {
                    ...
                    {
                        console.log(items.rate_per_gram);
            ...
            "aoColumns": [
				...
				},
                {
                    "mDataProp": function ( row, type, val, meta ) 
                    {
                        ...
                        action_content=(row.status==0 && access.edit == 1 ? '<a href="'+edit_url+'" class="btn btn-info btn-edit" data-id='+id+' title="QC Issue Edit"><i class="fa fa-edit"></i></a>' :(row.is_lot_created==0 &&access.edit == 1 ? '<a href="'+edit_receipt_url+'" class="btn btn-primary btn-edit" data-id='+id+' title="QC Receipt Edit"><i class="fa fa-edit"></i></a>':""));
                        return action_content;
                    }
                } 
		...
		{  	
            ...
            "aoColumns": [
                ...
                { "mDataProp": function ( row, type, val, meta ) {
                    ...
                    action_content=(row.pay_status==1 && access.edit == 1 ? '<a href="'+edit_url+'" class="btn btn-primary btn-edit" data-id='+id+' ><i class="fa fa-edit"></i></a><a href="'+print_url+'" target="_blank" class="btn btn-info btn-print" data-toggle="tooltip" title="Payment Copy"><i class="fa fa-print" ></i></a><button class="btn btn-warning" onclick="confirm_bill_cancel('+id+')"><i class="fa fa-close" ></i></button>' :'-');
                    return action_content;
                }
                }
...
function calculate_rate_fixing_total_paymnet(curRow)
{
    ...
    curRow.find('.payable_amt').val(parseFloat(parseFloat(rate_fix) * parseFloat(fix_weight)).toFixed(2));
}
...
function calculation_supplier_rate_cut() {
    $('#supplier_rate_cut_submit').prop('disabled',false);
    var src_type = $("input[name='supplier_rate_cut[rate_cut_type]']:checked").val();//1-Amount to weight , 2-Weight to Amount
    ...
    if (src_type == 1) {
        ...
        var total_amount    = parseFloat(parseFloat(taxable_amount)+parseFloat(taxable_amount)).toFixed(2);
        $('#taxable_amt').val(taxable_amount);
        $('#tax_amount').val(tax_amount);
        $('#src_amount').val(total_amount);
...
function calculate_supplier_rate_cut_payment_cost()
{
    var bal_amount             =0;
    var total_amount           =0;
	var receive_amount         =($('.src_amount ').val()!='' ? $('.src_amount ').val():0);
	var cash_amount            = ($('.cash_amount').val()!='' ? $('.cash_amount').val():0);
	var net_banking_pay        =($('#tot_net_banking_amt').html()!='' ? $('#tot_net_banking_amt').html():0);
	if(receive_amount>0)
	{
        			...
        			{ 
        				if(data.length>0){
        					...
        					for( var i=0; i<=data.length-1;i++){
        						...
        						$(api.column(15).footer()).html(money_format_india(parseFloat(lot_pcs).toFixed(0)));
        						...
        						$(api.column(16).footer()).html(money_format_india(parseFloat(lot_gwt).toFixed(3)));
        						...
        						$(api.column(17).footer()).html(money_format_india(parseFloat(lot_nwt).toFixed(3)));
        						...
        						$(api.column(18).footer()).html(money_format_india(parseFloat(lot_diawt).toFixed(3)));
        						...
        						$(api.column(19).footer()).html(money_format_india(parseFloat(tag_pcs).toFixed(0)));
        						...
        						$(api.column(20).footer()).html(money_format_india(parseFloat(tag_gwt).toFixed(3)));
        						...
        						$(api.column(21).footer()).html(money_format_india(parseFloat(tag_nwt).toFixed(3)));
        						...
        						$(api.column(22).footer()).html(money_format_india(parseFloat(tagdia_wt).toFixed(3)));
        						...
        						$(api.column(23).footer()).html(money_format_india(parseFloat(blc_pcs).toFixed(0)));
        						...
        						$(api.column(24).footer()).html(money_format_india(parseFloat(blc_gwt).toFixed(3)));
        						...
        						$(api.column(25).footer()).html(money_format_india(parseFloat(blc_nwt).toFixed(3)));
        						...
        						$(api.column(26).footer()).html(money_format_india(parseFloat(blc_diawt).toFixed(3)));
